...
// basit RAM cache (deploy/ölçek değişiminde sıfırlanır – Deno Deploy için yeterli)
...
Deno.serve(async (req) => {
  ...
  if (url.pathname === "/rank") {
    ...
    if (!apiKey) return new Response("Server misconfigured (missing HENRIK_API_KEY).", { status: 500 });
    ...
    if (!res.ok) {
      // upstream sorunlu: varsa son iyi değeri döndürelim
